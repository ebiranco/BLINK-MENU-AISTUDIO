...
const AIStudio: React.FC<AIStudioProps> = ({ language, restaurantId }) => {
    ...
    return (
        <div>
            ...
            {activeTab === 'photo' && <PhotographyStudio language={language} restaurantId={restaurantId} />}
            {activeTab === 'video' && <VideoStudio language={language} restaurantId={restaurantId} />}
        </div>
    );
};
...
const PhotographyStudio: React.FC<{ language: Language, restaurantId: string }> = ({ language, restaurantId }) => {
    ...
    const handleGenerateImage = async () => {
        ...
        try {
            ...
            if (imagePart && imagePart.inlineData) {
                ...
                // Save the generated image to our backend
                const saveResponse = await fetch(`${API_BASE_URL}/media/generated`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ imageData: b64Data, type: 'image', restaurantId }),
                });
                if(!saveResponse.ok) throw new Error('Failed to save media asset to backend.');
                // For the purpose of this app, we can just display the base64 directly
                const imageUrl = `data:${imagePart.inlineData.mimeType};base64,${b64Data}`;
     ...
     const handleCopyToClipboard = () => {
        if (generatedImage) {
            // In a real app, this would be a persistent URL from the backend
...
const VideoStudio: React.FC<{ language: Language, restaurantId: string }> = ({ language, restaurantId }) => {
    const [productImage, setProductImage] = useState<File | null>(null);
    ...
    const handleGenerateVideo = async () => {
        ...
        try {
            ...
            if (downloadLink) {
                 ...
                 // Save a reference to our backend
                 await fetch(`${API_BASE_URL}/media/generated`, {
                    method: 'POST',
                    headers: { 'Content-Type': 'application/json' },
                    body: JSON.stringify({ videoData: 'placeholder', type: 'video', restaurantId }), // Sending placeholder
                });
